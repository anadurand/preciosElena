...
const stationDetail = (update) => {
  ...
  const pDecena = $('<p class="direccion">Unidad:' + state.selectedStation.Decena + '</p>');
  const pDocena = $('<p class="direccion">Decena:' + state.selectedStation.Docena + '</p>');
  const pCiento = $('<p class="direccion">Docena:' + state.selectedStation.Ciento + '</p>');
